...
MAIN: {
  ...
  my $root           = '/usr/local/psortb';
  my $blastdir       = '/usr/local/src/ncbi-blast-2.6.0+/bin';
...
sub make_seq_input_uppercase {
    ...
    my @seqs2uc = split(/
>/, "
" . $seqinput); # Fixed: split only on > at start of line to handle > in headers (e.g. EC=> notation)
    shift(@seqs2uc); # remove blank first element
    ...
    foreach my $header_seq (@seqs2uc) {
      ...
      $s1 =~ s/[\n\r]+$//;  # strip trailing newlines to avoid doubling
      $uc_seqs .= ">$h1\n$s1\n";  # ensure exactly one trailing newline per entry
    }
